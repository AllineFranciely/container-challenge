...
import MovimentacoesModel from '../models/movimentacoes.models';
import Movimentacao from '../Interfaces/movimentacoes.interface';
import { BadRequestError, NotFoundError } from 'restify-errors';

const properties = ['tipo', 'dataInicio', 'dataFim', 'container'];

class MovimentacoesService {
  public model: MovimentacoesModel;

  constructor() {
    this.model = new MovimentacoesModel(connection);
  }
  ...
  static validationMovimentacao(movimentacao: Movimentacao): void | string {
  ...
  public create(movimentacao: Movimentacao): Promise<Movimentacao> {
    const isValidMovimentacao = MovimentacoesService.validationMovimentacao(movimentacao);
